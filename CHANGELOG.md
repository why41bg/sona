...
- **匿名队友名称增强** — 英雄选择阶段会显示“真实名称（客户端匿名代号）”并附带锁图标，换楼后也会同步更新。
...
- **匿名模式战绩分析** — 还原英雄选择阶段的匿名 PUUID，并使用混淆标识稳定匹配队友，支持查询战绩、战力评分和换楼同步。
- **匿名玩家身份回填** — 通过真实 PUUID 补全 Riot ID；即使战绩请求失败，战绩弹窗标题也会保留正确名称。
- **对局分析辨识度** — 调亮最近五局的胜负底色，开黑标记由 `A/B` 调整为 `A队/B队`。
...
- **自动 Ban 偶发失效** — 同时监听游戏阶段和选人会话，增加 Ban 结果回读、两步提交兜底、断线重试及配置热更新。
- **特殊模式配装缺少装备** — 修复 ARAM.GG 页面结构变化后本地配装只剩鞋子或装备图标为空的问题。
